...
-- 3. Desprogramar de forma segura cualquier cron previo si existe sin lanzar errores si no existe
DO $$
BEGIN
  PERFORM cron.unschedule('morning-brief-cron');
EXCEPTION
  WHEN OTHERS THEN
    -- Ignorar si la tarea no existe o no se puede desprogramar
    NULL;
END;
$$;
